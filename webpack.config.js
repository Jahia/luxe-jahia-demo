...
const clientConfig = {
    ...
    plugins: [
        ...
        !isWatchMode && new CycloneDxWebpackPlugin(cycloneDxWebpackPluginOptions),
...
function resetBuildErrors() {
    buildErrors = 0;
}

async function packAndDeploy() {

    console.log("buildErrors before packAndDeploy :", buildErrors)
    // If no build errors, pack and deploy the module
    if(buildErrors === 0) {
        resetBuildErrors()

        // We need to return a promise here, otherwise the WebpackShellPluginNext script won't be "blocking"
        return new Promise((resolve, reject) => {
            const child = spawn(`yarn jahia-pack && yarn jahia-deploy`, { shell: true });

            // Pipe the console output (stdout) to the current process
            ...
            child.on('close', (code) => {
                if (code === 0) {
                    ...
                    reject(new Error(`Jahia pack & deploy command failed with exit code ${code}`));
                }
            });
        });
    }
...
// Array of configs to build
const configs = [clientConfig, cssConfig, serverConfig];
